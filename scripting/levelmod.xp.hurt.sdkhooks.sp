...
public Action:OnTakeDamage(victim, &attacker, &inflictor, &Float:damage, &damagetype)
{
	if(lm_IsEnabled())
	{
		...
		if(amount > 0 && attacker > 0 && attacker <= MaxClients && IsClientInGame(attacker) && attacker != victim && GetClientTeam(attacker) != GetClientTeam(victim) && lm_GetClientLevel(attacker) < lm_GetLevelMax())
		{
